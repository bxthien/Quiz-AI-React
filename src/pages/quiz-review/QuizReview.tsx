...
const QuizReview = () => {
  ...
  const { quizData, userAnswers, correctCount, isModalOpen, setIsModalOpen } = useQuizReview();

  return (
    <div className={`text-left text-sm md:text-lg`}>
      <div className="max-w-2xl mx-auto p-4">
        <div className="static bg-[#BDE0FE] rounded-lg shadow-lg p-6 text-left">
          <img alt="Quiz logo" className="absolute inset-x-0 mx-auto w-30 top-5 md:top-0" src="./logo.svg" />
          <h1 className="text-3xl text-center font-bold mb-2 mt-10">
            You answered {correctCount}/{quizData.length} questions correctly!
          </h1>
          <div className="flex justify-center space-x-4 mt-4">
            <button className="bg-[url('./home-btn.svg')] w-30 h-15 bg-contain bg-no-repeat" onClick={() => navigate("/")}></button>
            <button className="bg-[url('./save-btn.svg')] w-30 h-15 bg-contain bg-no-repeat" onClick={() => setIsModalOpen(true)}></button>
          </div>
        </div>
            ...
            {quizData.map((questionData, index) => {
              ...
              return (
                <div key={index} className="bg-[#CAF0F8] rounded-lg p-2 mb-4">
                  <p className="text-sm md:text-lg font-bold mb-4">{question}</p>
                    ...
                    <ul className="list-none mb-4 text-sm md:text-lg">
                      {Object.entries(answers).map(([key, value]) => {
                        ...
                        let answerClass = isUserChoice
                          ? isCorrectChoice ? "text-green-600 font-bold" : "text-red-600 font-bold"
                          : isCorrectChoice ? "text-green-600 font-bold" : "";

                        return (
                          <li key={key} className={`mb-2 ${answerClass}`}>
                            {isUserChoice && <img src={isCorrectChoice ? "./true.svg" : "./false.svg"} className="mr-1 inline-block" />}
                            {key}. {value}
                          </li>
                        );
                      })}
                    </ul>
                    <img alt="Review icon" className="w-15 md:w-25" src={isCorrect ? "./correct.svg" : "./wrong.svg"} />
                  </div>
